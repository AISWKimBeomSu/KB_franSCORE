...
def build_grade_history(cfg: dict) -> pd.DataFrame:
    """실적연도별 **그때의 등급** 표 — 은행 연체 자료로 등급을 검증할 때 쓴다.

    왜 필요한가
        은행이 2024년에 취급한 가맹점주 대출의 연체를 설명하려면, 그 대출을 **2024년에
        볼 수 있었던 등급**(2022년 실적 — 2023년 정보공개서)에 맞춰야 한다. 최신 등급을 붙이면
        대출 이후에 나온 정보로 과거를 평가하는 셈이라(look-ahead) 결과가 낙관적으로 나온다.
