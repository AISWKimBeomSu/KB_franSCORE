...
def r_hq_no_data(ctx: Ctx) -> Finding | None:
    ...
    return Finding(
        ...
        # ⚠️ 예전 문구는 "외부감사 대상이 아니어서 감사보고서를 제출하지 않고"라고 단정했다.
        #    실제로는 법인번호로 확정 매칭된 본부가 적격 1,650개 중 248개뿐이라, 이 소견이
        #    붙는 브랜드 대부분은 '비대상'이 아니라 '매칭하지 못함'이다. 확인한 사실만 적는다.
        detail=("이 브랜드 가맹본부의 재무를 금융감독원 전자공시(감사보고서)와 "
                "공정위 정보공개서 열람분에서 확인하지 못했습니다(외부감사 비대상이거나 "
                "법인 매칭 실패). 본부의 자본잠식·적자 여부를 알 수 없으므로, 여신 심사 시 "
                "본부 최근 3개년 재무제표를 별도로 징구해 확인해야 합니다."),
