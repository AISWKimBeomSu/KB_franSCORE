...
_STEPS = (
    ...
    ("2", "무엇이 '나빠진 것'인지 정의한다",
     "새 지표를 만들지 않습니다. 공정위 공시 지표가 <b>업종 안에서 하위 구간에 들어가는 것</b>을 "
...
def render() -> None:
    ...
    # ── 어디에 쓰는가 ────────────────────────────────────────────
    # 실무자의 첫 질문은 "한도·금리에 안 쓴다면 그럼 어디에 쓰나"다 — 업무 단계별로 답한다.
    st.write("")
    st.markdown("### 어디에 쓰는가 — 여신 업무별")
    uses = (
        ("신규 취급 참고", "가맹점주 대출 신청이 들어왔을 때",
         "신청인이 속한 브랜드의 등급·소견·본부 재무를 확인하고, 참고의견서를 품의서에 첨부합니다. "
         "신청 목록은 <b>일괄 조회</b>로 한 번에 붙입니다."),
        ("협약대출 브랜드 관리", "가맹본부와 대출 협약을 맺거나 갱신할 때",
         "협약 후보 브랜드를 <b>브랜드 탐색·비교</b>로 나란히 놓고 보고, 협약 브랜드의 중대 신호와 "
         "등급 변화를 정기 재심사에 씁니다. 협약 한도·우대금리 자체를 정하는 데는 쓰지 않습니다."),
        ("사후관리 우선순위", "무엇부터 점검할지 정할 때",
         "<b>점검 큐</b>가 중대 신호 → 1년 내 악화 위험 × 가맹점 수 순서로 줄을 세우고, "
         "담당·처리상태·경위를 남깁니다."),
        ("편중 관리", "어느 브랜드에 여신이 쏠렸는지 볼 때",
         "<b>여신 포트폴리오</b>에서 브랜드·업종 한도 초과와 꼬리손실 기여를 봅니다. "
         "행내 여신 장부를 올리면 그 장부 기준으로 다시 계산합니다."),
        ("심사 반영 전 검증", "등급을 참고 지표에서 심사 지표로 올리기 전에",
         "<b>등급 검증</b>에 행내 가맹점주 대출·연체 자료를 올리면 취급 당시 등급 기준으로 "
         "등급별 연체율, 서열성, 내부등급 대비 추가 변별력을 판정합니다."),
    )
    st.markdown("".join(
        f"<div style='display:flex;gap:14px;padding:10px 0;border-bottom:1px solid {theme.BORDER}'>"
        f"<div style='min-width:150px'><b style='color:{theme.INK}'>{t}</b>"
        f"<div style='font-size:{theme.FS_SM};color:{theme.TEXT_MUTED};margin-top:2px'>{w}</div></div>"
        f"<div style='color:{theme.TEXT_SUB};line-height:1.65'>{d}</div></div>" for t, w, d in uses),
        unsafe_allow_html=True)
    ...
    guide = {
        "FRANSCORE": "전체 현황과 브랜드별 상세. 어떤 브랜드든 눌러 들어가면 진단 소견·"
                     "부문별 점검·공시 추이·본부 재무·검색 수요를 볼 수 있고, 참고의견서를 내려받습니다.",
        "브랜드 탐색·비교": "업종·등급·상태·규모·중대 신호로 거르고, 후보 브랜드 2~4개를 나란히 비교합니다.",
        "일괄 조회": "신청 목록(엑셀·CSV)을 올리면 브랜드를 찾아 등급·중대 신호·확인 서류를 붙여 돌려줍니다.",
        "점검 큐": "확인해야 할 브랜드를 담당자에게 배정하고 확인 결과를 기록합니다. "
                   "엑셀로 내려받아 결재에 붙일 수 있습니다.",
        "여신 포트폴리오": "브랜드별 여신 쏠림과 예상 손실을 봅니다. 은행 여신 장부를 올리면 그 장부로 "
                          "다시 계산하고, 담보유형별 손실률(LGD)로 같은 브랜드 차주가 함께 무너지는 해의 "
                          "꼬리손실까지 봅니다. 신규 실행·회수를 넣으면 즉시 재계산됩니다.",
        "등급 검증": "은행 연체 자료를 올리면 등급이 실제 연체를 서열화하는지, 내부등급에 없는 "
                    "정보를 더하는지 판정합니다. 가상 자료로 절차를 먼저 볼 수 있습니다.",
    ...
    with st.expander("이 도구를 쓰면 안 되는 곳"):
        st.markdown(
            ...
            "- 등급을 제3자 자료·광고·보도에 옮겨 싣는 것(인용·재배포)")
        st.caption("2선 리스크 관리의 **점검 우선순위**와 **심사 참고 정보**가 정해진 용도입니다.")

    # 실명으로 공개하는 이상, 브랜드가 이의를 제기할 창구를 화면에 열어 둔다 (모형 사용 명세 §3).
    st.caption(
        "이 화면은 공정위 공시·감사보고서 등 공개 자료를 다시 계산한 **개인 연구 프로젝트**이며 "
        "신용평가가 아닙니다. 브랜드 관련 정정·삭제 요청은 "
        "[저장소 이슈](https://github.com/AISWKimBeomSu/KB_franSCORE/issues)로 남겨 주시면 "
        "해당 브랜드를 공개 화면에서 내리거나 가명으로 전환합니다.")
