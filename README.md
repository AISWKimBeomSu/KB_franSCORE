...
<sub>화면 속 브랜드명은 <b>가명</b>입니다(예: 커피 685) — 모형 사용 명세 §3(실명 브랜드 등급 공표 금지)에 따라 공개 데모와 README 는 가명으로 표시하고, 수치는 실제 공시 그대로 둡니다.</sub>
...
<sub>모든 그림은 공개 데모 화면입니다 — 브랜드는 가명, 숫자는 실제 공시입니다.</sub>
...
- 한 달에 영업 점포의 30% 이상이 한꺼번에 폐업 처리되면 '악화'가 아니라 **'확인 필요'** 로 둡니다 — 상호 변경·재인허가·행정 정리일 수 있어 원자료부터 봐야 합니다(실제로 한 브랜드에서 184곳 중 132곳이 한 달에 폐업 처리됐습니다).
...
**품질 장치** — 시점 누출 자동 검사 · 라벨 규칙 학습 전 동결 · 두 표본 트랙 사전 선언 · 문서에 적힌 수치 **204건**을 산출물과 자동 대조(`tools/check_doc_numbers.py`) · pytest 전 화면 스모크 테스트 · 공개 모드 실명 누출 검사(공개용 사본 전체 + 전 화면) · 푸시마다 GitHub Actions CI
...
| **공개 배포** | 공개 데모는 브랜드·본부 이름과 식별번호를 가명으로 바꾼 사본을 읽습니다(`src/public.py`). 설정 경로 한 곳에서 전환해 화면 코드는 그대로입니다. |
...
FRANSCORE_PUBLIC_DEMO=1 streamlit run src/app.py   # 공개 데모와 같은 가명 화면
```
...
│  ├─ public.py                             # 공개 데모 가명 사본 (모형 사용 명세 §3)
...
- 공개 데모는 브랜드를 **가명**으로 보여 주지만 숫자는 실제 공시라, 공정위 자료와 대조하면 큰 브랜드는 다시 알아볼 수 있습니다 — 재식별을 막는 익명화가 아니라 실명 등급을 공표하지 않는 조치입니다. 특정 브랜드에 대한 평가·권유가 아니며 등급을 인용·배포하지 않아야 합니다. 저장소의 `outputs/` 는 재현용 연구 자료입니다.
